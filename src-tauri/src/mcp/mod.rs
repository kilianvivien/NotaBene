//! Agent access: an embedded MCP server on loopback, plus the Tauri commands
...
/// Start (or restart with fresh settings) the embedded server. Returns the
/// port actually bound, which may differ from the preferred one.
#[tauri::command]
...
/// Webview answer to a forwarded `notabene-mcp-request` event.
#[tauri::command]
