...
accept Apple's command-line model terms once with `sudo fm license`, then enable
Apple Intelligence in Settings ▸ AI providers. NotaBene starts and stops the
system model with the app, entirely on-device, with no key or model download.
Its 8,192-token context is suited to short notes and selections;
