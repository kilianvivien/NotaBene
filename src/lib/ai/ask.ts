 ...
 *
 * The one AI feature here that writes nothing at all: it reads the note, holds
 * a short conversation about it, and stops. That is what lets it stream
 * straight into the panel with no diff gate — there is nothing to gate.
 *
 * Grounded in the note by construction. The whole note goes into the system
 * message, so there is no retrieval step to get wrong on a corpus this size; a
 * lecture note is a few thousand tokens and chunking it would only introduce a
 * way to miss the paragraph the student was asking about.
 */
